...
import static com.gakshintala.bookmybook.repositories.DBUtils.insertAndGetGeneratedKey;
import static com.gakshintala.bookmybook.repositories.DBUtils.performUpdateOperation;
...
class LibraryRepository implements PersistBookInLibrary, FindAvailableBook, FindBookOnHold, HandlePatronEventInLibrary {
    ...
    public Try<CatalogueBookInstanceId> handle(PatronEvent event) {
        return Match(event).of(
                ...
                Case($(instanceOf(PatronEvent.BookPlacedOnHoldNow.class)), this::handle),
    ...
    private Try<CatalogueBookInstanceId> handle(PatronEvent.BookPlacedOnHold bookPlacedOnHold) {
        return this.handle(bookPlacedOnHold.getBookPlacedOnHoldNow());
    }

    private Try<CatalogueBookInstanceId> handle(PatronEvent.BookPlacedOnHoldNow bookPlacedOnHoldNow) {
    ...
    private Try<CatalogueBookInstanceId> handle(PatronEvent.BookCollected bookCollected) {
    ...
    private Try<CatalogueBookInstanceId> handle(PatronEvent.BookHoldExpired holdExpired) {
    ...
    private Try<CatalogueBookInstanceId> handle(PatronEvent.BookHoldCanceled holdCanceled) {
    ...
    private Option<? extends Book> handleBookPlacedOnHold(Book book, PatronEvent.BookPlacedOnHoldNow bookPlacedOnHoldNow) {
    ...
    private Option<BookOnHold> bookNotAlreadyOnHoldByPatron(BookOnHold onHold, PatronEvent.BookPlacedOnHoldNow bookPlacedOnHoldNow) {
        return Option(onHold)
                .filter(onHoldBook -> !onHoldBook.by(new PatronId(bookPlacedOnHoldNow.getPatronId())));
    }

    private Book handleBookHoldExpired(Book book, PatronEvent.BookHoldExpired holdExpired) {
        return Match(book).of(
    ...
    private Book handleBookHoldCanceled(Book book, PatronEvent.BookHoldCanceled holdCanceled) {
        return Match(book).of(
    ...
    private Book handleBookCollected(Book book, PatronEvent.BookCollected bookCollected) {
        return Match(book).of(
